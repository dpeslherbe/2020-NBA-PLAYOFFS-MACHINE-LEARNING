...
model.glm <- rfe(datatrain[,(2:63)], as.factor(datatrain[,1]), rfeControl = cntrl, sizes = c(5:25), metric = "ROC", method = "glm", family = "binomial")
model.glm
...
model.glm <- glm(ytrain~ Ranking + blkPerGameTeam + pctEFGTeamMisc +
                   ...
                   ortgTeamMisc + pctTrueShootingeTeamMisc +
...
model.lda <- lda(ytrain~ Ranking + winsTeam + marginVictoryTeam +
                   ...
                   ortgTeamMisc + pctEFGTeamOppMisc + pctTrueShootingeTeamMisc +
...
model.knn <- knn3(formula = as.factor(ytrain)~ Ranking + winsTeam + marginVictoryTeam +
                    ...
                    pctEFGTeamOppMisc + pctTrueShootingeTeamMisc +
...
##Discriminant Analysis Model suggests the Boston Celtics, Brooklyn Nets, Denver Nuggets,
##Houston Rockets, Indiana Pacers, LA Clippers, Los Angeles Lakers, Miami Heat,
##Milwaukee Bucks, Oklahoma City Thunder, Orlando Magic, Philadelphia 76ers,
